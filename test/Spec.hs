...
main = hspec $ do
  ...
  describe "GraphQL" $ do
    ...
    it "updates headline titles via mutation" $ do
      let query =
            "{\"authorization\":\"Bearer secret\",\"query\":\"mutation { updateHeadlineTitle(path: \\\"mutation-headline.org\\\", id: \\\"hello\\\", title: \\\"Updated\\\") }\"}"
          filePath = "test/fixtures/mutation-headline.org"
          cleanup = do
            exists <- doesFileExist filePath
            when exists (removeFile filePath)
      bracket_ cleanup cleanup $
        bracket_ (setEnv "ORG_BACKEND_ORG_DIR" "test/fixtures") (unsetEnv "ORG_BACKEND_ORG_DIR") $
          bracket_ (setEnv "ORG_BACKEND_TOKEN" "secret") (unsetEnv "ORG_BACKEND_TOKEN") $ do
            ...
            result <- GraphQL.execute (pack query)
            unpack result `shouldBe` "{\"data\":{\"updateHeadlineTitle\":true}}"
            content <- readFile filePath
            content `shouldBe` "* Updated\n"

    it "preserves startup options when updating headlines" $ do
      let query =
            "{\"authorization\":\"Bearer secret\",\"query\":\"mutation { updateHeadlineTitle(path: \\\"mutation-headline-startup.org\\\", id: \\\"hello\\\", title: \\\"Updated\\\") }\"}"
          filePath = "test/fixtures/mutation-headline-startup.org"
          cleanup = do
            exists <- doesFileExist filePath
            when exists (removeFile filePath)
      bracket_ cleanup cleanup $
        bracket_ (setEnv "ORG_BACKEND_ORG_DIR" "test/fixtures") (unsetEnv "ORG_BACKEND_ORG_DIR") $
          bracket_ (setEnv "ORG_BACKEND_TOKEN" "secret") (unsetEnv "ORG_BACKEND_TOKEN") $ do
            writeFile filePath "#+STARTUP: overview\n* Hello\n"
            result <- GraphQL.execute (pack query)
            unpack result `shouldBe` "{\"data\":{\"updateHeadlineTitle\":true}}"
            content <- readFile filePath
            content `shouldBe` "#+STARTUP: overview\n* Updated\n"
    ...
    it "updates headline todo via mutation" $ do
      let query =
          ...
          filePath = "test/fixtures/mutation-todo.org"
          cleanup = do
            exists <- doesFileExist filePath
            when exists (removeFile filePath)
      bracket_ cleanup cleanup $
        bracket_ (setEnv "ORG_BACKEND_ORG_DIR" "test/fixtures") (unsetEnv "ORG_BACKEND_ORG_DIR") $
          bracket_ (setEnv "ORG_BACKEND_TOKEN" "secret") (unsetEnv "ORG_BACKEND_TOKEN") $ do
            writeFile filePath "* TODO Hello\n"
            result <- GraphQL.execute (pack query)
            unpack result `shouldBe` "{\"data\":{\"updateHeadlineTodo\":true}}"
            content <- readFile filePath
    ...
    it "updates headline scheduled timestamp via mutation" $ do
      let query =
            "{\"authorization\":\"Bearer secret\",\"query\":\"mutation { updateHeadlineScheduled(path: \\\"mutation-scheduled.org\\\", id: \\\"hello\\\", scheduled: \\\"2024-01-02 Tue\\\") }\"}"
          filePath = "test/fixtures/mutation-scheduled.org"
          cleanup = do
            exists <- doesFileExist filePath
            when exists (removeFile filePath)
      bracket_ cleanup cleanup $
        bracket_ (setEnv "ORG_BACKEND_ORG_DIR" "test/fixtures") (unsetEnv "ORG_BACKEND_ORG_DIR") $
          bracket_ (setEnv "ORG_BACKEND_TOKEN" "secret") (unsetEnv "ORG_BACKEND_TOKEN") $ do
            writeFile filePath "* Hello\n"
            result <- GraphQL.execute (pack query)
            unpack result `shouldBe` "{\"data\":{\"updateHeadlineScheduled\":true}}"
            content <- readFile filePath
